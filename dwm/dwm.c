...
createnotch(Client *c)
{
	...
	if (c->nomaximalist)
		return; /* excluded via Rule.nomaximalist: no notch, ever */
...
updatenotchpos(Client *c)
{
	...
	if (!c->twin)
		return;
	wc.x = c->x;
	wc.y = c->y - bh - 2 * c->bw; /* notch's outer bottom edge lands exactly on the client's outer top edge */
	wc.width = MAX((int)c->w, 1);
	wc.height = bh;
	wc.border_width = c->bw; /* mirrors the client's border so the two form one continuous outline */
...
ensurenotchroom(Client *c)
{
	...
	miny = c->mon->my + bh + 2 * c->bw; /* leave room for the notch (plus its border) above the window */
	if (c->y < miny)
